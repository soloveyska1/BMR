...
SPAM_KEYWORDS = [
    ...
    "в час", "в день", "в неделю",  # оплата
    "оплата сразу", "оплата ежедневно", "оплата в тот же день",
    "свободный график", "гибкий график",
...
SPAM_PATTERNS = [
    # Деньги и оплата
    r"(?:от|до)\s*\d+\s*(?:₽|руб|рублей|р|к|тыс|тысяч|\$|долл|euro|евро)",  # от 3500 руб, от 50к
    r"\d+\s*(?:₽|руб|рублей)\s*(?:в\s*)?(?:час|день|неделю|месяц)",  # 3500 руб в час
    r"(?:от|до)\s*\d{3,}\s*(?:в\s*)?(?:час|день|неделю)",  # от 3500 в час
    r"\d{4,}\s*(?:₽|руб|рублей|р\.)",  # 3500 рублей (4+ цифры)

    # Призывы к действию
    r"(?:пиш[иу]|напиш[иу])(?:те)?\s*(?:в\s*)?(?:лс|личк|дм|dm|директ)?",  # пишите, пиши в лс
    r"(?:звон[и|я]|позвон)[и|я]?(?:те)?",  # звоните, звони, позвоните
    r"(?:звоните|пишите)\s*(?:звоните|пишите)",  # звоните пишите

    # Телефоны (российские)
    r"(?:\+7|8)[\s\-]?\(?9\d{2}\)?[\s\-]?\d{3}[\s\-]?\d{2}[\s\-]?\d{2}",  # +79534014601, 8-953-401-46-01
    r"\+?\d{10,12}",  # любой длинный номер
    ...
    # Работа/услуги
    r"требу[юе]тся?\s*\d+",  # требуется 2, требуются 5
    r"\d+\s*(?:человек|людей|чел)\s*(?:на|для|в)",  # 2 человека на завтра
    r"(?:нужн[ыо]|ищем|ищу|набираем)\s*\d*\s*(?:человек|людей|чел|сотрудник)",  # нужны 2 человека
    r"(?:готов[ыа]?|мог[у|ем])\s+(?:выйти|выехать|приехать|помочь)",  # готовы выйти
    r"(?:любые|все|всякие)\s*(?:работы|услуги)",  # любые работы
    r"\d+[\-‑–]\d+\s*(?:человек|чел|людей)",  # 2-3 человека
]
...
def is_spam(text: str) -> tuple[bool, str]:
    ...
    # Проверка комбинаций (2+ слов = спам)
    combo_count = 0
    found_combos = []
    ...
    # Проверка: номер телефона + призыв = спам
