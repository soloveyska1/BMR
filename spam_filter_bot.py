...
SPAM_KEYWORDS = [
    ...
    "оплата от", "нужны водител", "нужен водител", "требуются водител",
...
SPAM_PATTERNS = [
    ...
    r"\d{4,}р\s",  # 8000р без точки

    # Призывы в ЛС
    r"(?:пиш[иу]|напиш[иу])(?:те)?\s*(?:в\s*)?(?:лс|личк|л\.с\.|дм|dm|директ)",
    r"(?:пиш[иу]|напиш[иу])(?:те)?\s*$",  # "пишите" в конце сообщения
...
PROMO_CONTEXT_KEYWORDS = [
    "книг", "гайд", "курс", "обучен", "урок", "методичк",
    "рекоменд", "совету", "полезн", "интересн", "поделил",
    "рассказал", "рассказала", "совет", "обзор",
    "товарищ", "знакомый", "простым языком", "подводн", "направлен",
]
...
def normalize_text(text: str) -> str:
    ...
    # Спамерские замены букв
    spam_replacements = {
        'пиш': 'пиш',  # восстанавливаем "пишите"
        'ніш': 'пиш',  # украинская і
        'ниш': 'пиш',  # после n→н
        'dен': 'ден',  # dень → день
        'ден': 'ден',
    }
    for spam, fix in spam_replacements.items():
        text = text.replace(spam, fix)
